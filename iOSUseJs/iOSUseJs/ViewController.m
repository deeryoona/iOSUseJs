...
- (WKWebView *)webView {
    if (!_webView) {
        ...
        [_configuration.userContentController addScriptMessageHandler:weakScriptMessageDelegate name:@"tronTool.getSignWeight"];
...
- (void)viewDidDisappear:(BOOL)animated {
    ...
    [self.webView.configuration.userContentController removeScriptMessageHandlerForName:@"tronTool.getSignWeight"];
}
...
//        }];
        [self.webView evaluateJavaScript:@"javascript:window.tronTool.testnet();" completionHandler:^(id _Nullable response, NSError * _Nullable error) {
            NSLog(@"initial value: %@, initial error: %@", response, error);
        }];
        [self testTron];
    });
...
//    [self.webView evaluateJavaScript:[NSString stringWithFormat:@"javascript:window.tronTool.delegateCall(window.tronTool.sign('%@', '%@'), 'tronTool.sign')",@"hello world", @"4594348e3482b751aa235b8e580efef69db465b3a291c5662ceda6459ed12e39"] completionHandler:^(id _Nullable response, NSError * _Nullable error) {}];
    ...
    [self.webView evaluateJavaScript:[NSString stringWithFormat:@"javascript:window.tronTool.signTx(%@, '%@')",txObj, @"4594348e3482b751aa235b8e580efef69db465b3a291c5662ceda6459ed12e39"] completionHandler:^(id _Nullable response, NSError * _Nullable error) {}];
    
//    [self.webView evaluateJavaScript:[NSString stringWithFormat:@"javascript:window.tronTool.getSignWeight(%@)",txObj] completionHandler:^(id _Nullable response, NSError * _Nullable error) {}];
}
